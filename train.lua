...
local make_model_fn = function(id)
    ...
    model:cuda()
    --for i, module in ipairs(model:network():listModules()) do
        --print(module)
    --end
    return model
end
...
--engine = tnt.ResumableDPOptimEngine(
    --config.ngpus, config.binaryWeight, thread_init_fn, make_model_fn, make_criterion_fn
--)
engine = tnt.ResumableDPOptimEngine(
...
local log = tnt.Log{
    keys = {"time", "trainLoss", "validLoss", "testLoss"},
    onFlush = {
        logtext{
            filename=plpath.join(config.savedir, 'log.txt'),
            keys={"time", "trainLoss", "validLoss", "testLoss"}
        }
    }
}
