...
FConvModel.makeTemporalConv = argcheck{
    ...
    call = function(self, binaryConf, ninput, noutput, kwidth, pad, doin)
        local conv
        if binaryConf.binaryWeight then
            conv = nn.Sequential()
            conv:add(BinaryTemporalConvolution(ninput, noutput, kwidth, 1, pad))
            conv:add(MyBatchNormalization(512, 1e-5, 0.1, false))
...
FConvModel.makeBottleneckConv = argcheck{
    ...
    {name='pad', type='number'},
    {name='doin', type='number', default=0},
    call = function(self, binaryConf, ninput, noutput, bnhid, kwidth, pad,
      doin)
        ...
        bottleneck:add(self:makeTemporalConv(
            binaryConf, bnhid, bnhid*2, kwidth, pad)
        )
        bottleneck:add(nn.GatedLinearUnit())
...
FConvModel.makeEncoderConvBlock = argcheck{
    ...
    {name='kwidth', type='number'},
    {name='doin', type='number', default=0},
    call = function(self, binaryConf, ninput, noutput, bfactor, kwidth, doin)
        ...
        if bfactor > 0 then
            ...
            conv = self:makeBottleneckConv(
                binaryConf, ninput, noutput * 2, bnhid, kwidth, pad, doin)
        else
            conv = self:makeTemporalConv(
                binaryConf, ninput, noutput * 2, kwidth, pad, doin)
        end
        block:add(conv)
...
FConvModel.makeDecoderConvBlock = argcheck{
    ...
    call = function(self, binaryConf, ninput, noutput, bfactor, kwidth, doin)
        ...
        block:add(self:makeTranspose(binaryConf))
        block:add(conv)
        block:add(self:makeTranspose(binaryConf))
...
FConvModel.makeEncoderStack = argcheck{
    ...
    call = function(self, config, nhids)
        ...
        stack:add(self:makeTranspose(config.binaryConf))
        for i = 1, #nhids do
            ...
            stack:add(nn.ConcatTable()
                :add(nn.Sequential()
                    ...
                    :add(self:makeEncoderConvBlock(config.binaryConf,
...
FConvModel.makeDecoderLM = argcheck{
    ...
    call = function(self, config)
        ...
        for i = 1, #config.nlmhids do
            ...
            lmOut = self:makeDecoderConvBlock(config.binaryConf,
