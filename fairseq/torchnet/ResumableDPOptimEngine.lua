...
ResumableDPOptimEngine.__init = argcheck{
    ...
    call = function(self, nshards, thread_init_fn,
        ...
        --self.binaryWeight = binaryWeight
        ...
        self.pool = threads.Threads(
            ...
            function(id)
                ...
                --_G.binaryWeight = self.binaryWeight
                --if _G.binaryWeight then
                    --_G.convNodes = _G.model:network():findModules('nn.TemporalConvolutionTBC')
                    --_G.realParams = _G.params:clone()
                    --function binarizeConvParams()
                        --for i =1, #convNodes do
                            --local n = convNodes[i].weight:nElement()
                            --local s = convNodes[i].weight:size()
                            --convNodes[i].m = convNodes[i].weight:norm(1,3):sum(2):sum(1):div(n):expand(s);
                            --convNodes[i].weight:sign():cmul(convNodes[i].m)
                        --end
                    --end

                    --function clampConvParams()
                        --for i =1, #convNodes do
                            --convNodes[i].weight:clamp(-1,1)
                        --end
                    --end

                    --function updateBinaryGradWeight()
                        --for i =1, #convNodes do
                            --local n = convNodes[i].weight:nElement()
                            ----local m = convNodes[i].m;
                            --convNodes[i].m[convNodes[i].weight:le(-1)]=0
                            --convNodes[i].m[convNodes[i].weight:ge(1)]=0
                            ----m:add(1/(n)):mul(1-1/s[2])
                            --convNodes[i].m:add(1/(n))
                            ----if opt.optimType == 'sgd' then
                            ----convNodes[i].m:mul(n);
                            ----end
                            --convNodes[i].gradWeight:cmul(convNodes[i].m)--:cmul(mg)
                        --end
                    --end
                --end
                ...
                if sample then
                    ...
                    self.pool:addjob(shardid,
                        function(sample)
                            ...
                            --if binaryWeight then
                                ----meanCenterConvParams()
                                --clampConvParams()
                                --_G.realParams:copy(_G.params)
                                --binarizeConvParams()
                            --end
